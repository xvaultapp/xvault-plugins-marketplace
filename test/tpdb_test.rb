...
# Ruby alone. tpdb.rb references ActiveSupport's blank?/present?/presence,
# Time.current, Integer#hours/#ago, and ::Metadata::SearchTerm — those only really exist inside
# the host XVault process (docs/scraper-plugin-authoring.md, "Connector escape
# hatch"). Here we hand-roll just enough of each to drive the plugin's pure
# logic without a network call or a Rails boot. Scene/VideoFile stand-ins are
# plain classes (no ActiveRecord) — resolve only ever touches the handful of
# attributes below.

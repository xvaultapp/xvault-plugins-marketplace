...
thumb fields use `cover_url` (not `image` — that's for `scene:` only). To power
`/watch` click-to-play, declare `search.embed` (`template` + `id_pattern`) so
each result carries an `embed_url`. See the plugins already in
[`plugins/`](./plugins) for worked examples, and the full field reference in
XVault's own
